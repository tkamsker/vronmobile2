#!/bin/bash
#
# Test BlenderAPI and download result automatically
#
# This script:
#   1. Runs simple_convert_test.py to create session and convert file
#   2. Extracts session ID from test output
#   3. Uses download_result.sh to download the converted GLB file
#   4. Optionally deletes the session
#
# Usage:
#   ./test_and_download.sh [BASE_URL] [API_KEY] [TEST_FILE]
#
# Example:
#   ./test_and_download.sh
#   ./test_and_download.sh https://blenderapi.stage.motorenflug.at dev-test-key-1234567890 test_files/scan.usdz
...
# Default values
DEFAULT_BASE_URL="https://blenderapi.stage.motorenflug.at"
DEFAULT_API_KEY="dev-test-key-1234567890"
DEFAULT_TEST_FILE="/Users/thomaskamsker/Documents/Atom/vron.one/microservices/blenderapi/test_files/scan_scan-1767108489607-489607.usdz"
...
if [ ! -f "./simple_convert_test.py" ]; then
    echo -e "${RED}Error: simple_convert_test.py not found in current directory${NC}"
    exit 1
fi

echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}BlenderAPI Test & Download${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""
...
echo -e "${BLUE}[1/3] Running conversion test...${NC}"
echo ""

# Run Python test and capture output
if python simple_convert_test.py "${BASE_URL}" "${API_KEY}" "${TEST_FILE}" 2>&1 | tee "${TEST_OUTPUT}"; then
    TEST_SUCCESS=true
else
    TEST_SUCCESS=false
fi
